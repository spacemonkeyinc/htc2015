...
def printCube():
  board = Board(cube_size=3)
  cells = 0
  while True:
    for marker in ["red", "blue"]:
      if cells >= board.cube_size ** 3:
        return False
      i = random.choice(range(board.cube_size))
      j = random.choice(range(board.cube_size))
      k = random.choice(range(board.cube_size))
      if board[(i, j, k)] is None:
        board[(i, j, k)] = marker
        cells += 1
        try: get_next_position(board)
        except Exception, e:
          continue
        for i in range(board.cube_size):
          for k in range(board.cube_size):
            for j in range(board.cube_size):
              sys.stdout.write(
                  {"blue": "X", "red": "O", None: "_"}[board[(i, j, k)]])
